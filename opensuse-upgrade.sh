...
params="--download in-advance --auto-agree-with-licenses --replacefiles --force-resolution --no-confirm"
...
z --releasever "$new" patch --no-recommends $params --updatestack-only
z --releasever "$new" dist-upgrade --no-recommends $params 
...
if ! [ "$2" = "--no-packman" ]; then
  ...
  z --releasever "$new" install --recommends $params --allow-vendor-change \
  ...
  z --releasever "$new" install --recommends $params \
